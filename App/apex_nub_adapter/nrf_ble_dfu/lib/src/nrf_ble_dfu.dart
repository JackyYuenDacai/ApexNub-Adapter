...
class NrfBleDfu {
  ...
  Future<void> _transferObject({
    ...
    await for (final event in controlPoint.lastValueStream) {
      ...
      /// Response Execute Error - Operation Not Permitted
      /// [60 04 08]
      if (event.elementAtOrNull(0) == NrfDfuOp.response.code &&
          event.elementAtOrNull(1) == NrfDfuOp.objectExecute.code &&
          event.elementAtOrNull(2) == NrfDfuResult.operationNotPermitted.code) {
        log('Object execute operation not permitted, retrying...');
        // 等待一段时间后重试
      ...
      /// Response Execute Error - Other errors
      /// [60 04 XX] where XX != 01 (success)
      if (event.elementAtOrNull(0) == NrfDfuOp.response.code &&
          event.elementAtOrNull(1) == NrfDfuOp.objectExecute.code &&
          event.elementAtOrNull(2) != NrfDfuResult.success.code) {
        final errorCode = event.elementAtOrNull(2) ?? 0;
        final errorResult = NrfDfuResult.values.firstWhere(
          (result) => result.code == errorCode,
          orElse: () => NrfDfuResult.invalid,
        );
        throw Exception('Object execute failed: ${errorResult.description} (code: 0x${errorCode.toRadixString(16).padLeft(2, '0')})');
      }
